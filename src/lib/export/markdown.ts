  ...
  const lines = [
    ...
    ...toList(
      plan.evidenceItems.map(
        (item) =>
          `[${item.title}](${item.url}) - ${item.source}（可信度：${item.credibility ?? "unverified"}）${item.citationReason ? `：${item.citationReason}` : ""}`,
      ),
    ),
  ];
  ...
  const renderGroup = (parentId: string | undefined, level: number): string[] => {
    return (byParent.get(parentId) ?? [])
      ...
      .flatMap((task) => {
        ...
        const deps = task.dependencyIds?.length ? `；依赖：${task.dependencyIds.join(", ")}` : "";
        return [
          `${prefix}- [${task.status === "done" ? "x" : " "}] ${task.title}（${task.priority}）${deps}`,
